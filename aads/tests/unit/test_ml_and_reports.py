...
class TestMLExperimentation:
    ...
    def test_ml_experiment_agent_and_reports(self, tmp_path, sample_ml_data):
        ...
        nb_dict = nb_agent.run(state, best_model_name=best_name)
        assert "notebook_generation" in state.completed_phases
        assert (mgr.get_path("notebooks") / "pipeline_notebook.ipynb").exists()

        # 5. Report Generator Agent
        rep_agent = ReportGeneratorAgent(artifact_manager=mgr)
        summary_md = rep_agent.run(state, best_model_name=best_name, best_metrics=best_metrics, eval_report=eval_report)
